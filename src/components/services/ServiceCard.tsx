...
function PhotoArea({ photos, title, layout }: { photos: string[]; title: string; layout: 'grid' | 'list' }) {
  ...
  return (
    <div className={containerClass}>
      ...
      {total > 1 && (
        <>
          <button onClick={prev} className="absolute left-1.5 top-1/2 -translate-y-1/2 flex size-7 items-center justify-center rounded-full bg-black/50 text-white opacity-0 transition-opacity group-hover:opacity-100 hover:bg-black/70" aria-label="Foto anterior">
            <ChevronLeft className="size-4" />
          </button>
          <button onClick={next} className="absolute right-1.5 top-1/2 -translate-y-1/2 flex size-7 items-center justify-center rounded-full bg-black/50 text-white opacity-0 transition-opacity group-hover:opacity-100 hover:bg-black/70" aria-label="Próxima foto">
...
export function ServiceCard({ service, categoryName, photos, layout = 'grid' }: ServiceCardProps) {
  const price = service.price_type === 'quote' || service.price === null
    ? 'A combinar'
              ...
              <div className="flex flex-wrap items-end justify-between gap-2">
                <div className="space-y-1">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {categoryName && <Badge variant="secondary" className="text-[10px]">{categoryName}</Badge>}
                  </div>
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <MapPin className="size-3" />{formatDistance(service.distance_km * 1000)}
                    </span>
        ...
        <CardContent className="space-y-2 p-4">
          ...
          <div className="flex items-center justify-between pt-1">
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <MapPin className="size-3.5" />{formatDistance(service.distance_km * 1000)}
            </div>
